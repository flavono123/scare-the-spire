...
function getComboResources(
  blocks: PostBlock[],
  entityMap: Map<string, EntityInfo>,
): EntityInfo[] {
  const resources: EntityInfo[] = [];
  const seen = new Set<string>();

  for (const block of blocks) {
    if (block.type !== "entity" && (block.type !== "keyword" || !block.entityId || !block.entityType)) {
      continue;
    }
...
export function ComboResourceStack({
  ...
  const allResources = useMemo(
    () => getComboResources(blocks, entityMap),
    [blocks, entityMap],
  );
  ...
  const stackWidth = itemWidth + expandedStep * (resources.length - 1);
