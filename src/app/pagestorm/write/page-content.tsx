...
import { PagestormWriteClient } from "@/components/pagestorm/pagestorm-client";
...
import { PAGESTORM_BACKGROUND_SRC } from "@/lib/pagestorm";
...
export async function generatePagestormWriteMetadata(
...
export async function renderPagestormWritePage(
  gameLocale: GameLocale = DEFAULT_ROUTE_GAME_LOCALE,
) {
  const gameCopy = await getPagestormGameCopy(gameLocale);

  return (
    <div className="relative isolate min-h-[calc(100svh-3rem)]" data-pagestorm-page="write">
      <ServiceBackground
        src={PAGESTORM_BACKGROUND_SRC}
        imageClassName="object-[42%_center] sm:object-center"
      />
      <div className={TOYBOX_WIDE_SHELL_CLASS}>
        <PagestormWriteClient gameCopy={gameCopy} />
      </div>
    </div>
  );
}
