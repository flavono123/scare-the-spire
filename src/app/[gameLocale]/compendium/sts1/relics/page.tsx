import BasePage, { generateMetadata as generateBaseMetadata } from "@/app/(codex)/_sts1/relics/page";
...
export default async function LocalizedSts1RelicsPage({ params }: Props) {
  const { gameLocale } = await getLocalePairFromParams(params);
  return BasePage({
    searchParams: Promise.resolve(searchRecordForGameLocale(gameLocale)),
  });
