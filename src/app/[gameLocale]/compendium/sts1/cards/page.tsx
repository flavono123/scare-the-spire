import BasePage, { generateMetadata as generateBaseMetadata } from "@/app/(codex)/_sts1/cards/page";
...
export default async function LocalizedSts1CardsPage({ params }: Props) {
  const { gameLocale } = await getLocalePairFromParams(params);
  return BasePage({
    searchParams: Promise.resolve(searchRecordForGameLocale(gameLocale)),
  });
