import BasePage, { generateMetadata as generateBaseMetadata } from "@/app/(codex)/_codex/badges/page";
import {
...
export default async function LocalizedBadgesPage({ params }: Props) {
  const { gameLocale } = await getLocalePairFromParams(params);
  return BasePage({
    searchParams: Promise.resolve(searchRecordForGameLocale(gameLocale)),
  });
