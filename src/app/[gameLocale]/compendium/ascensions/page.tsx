import BasePage, { generateMetadata as generateBaseMetadata } from "@/app/(codex)/_codex/ascensions/page";
import {
...
export default async function LocalizedAscensionsPage({ params }: Props) {
  const { gameLocale } = await getLocalePairFromParams(params);
  return BasePage({
    searchParams: Promise.resolve(searchRecordForGameLocale(gameLocale)),
  });
