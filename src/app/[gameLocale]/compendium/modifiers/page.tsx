import BasePage, { generateMetadata as generateBaseMetadata } from "@/app/(codex)/_codex/modifiers/page";
import {
...
export default async function LocalizedModifiersPage({ params }: Props) {
  const { gameLocale } = await getLocalePairFromParams(params);
  return BasePage({
    searchParams: Promise.resolve(searchRecordForGameLocale(gameLocale)),
  });
