...
export function getColorSchemeBootScript(): string {
  return `(function(){try{var k=${JSON.stringify(COLOR_SCHEME_STORAGE_KEY)};var darkMeta=${JSON.stringify(THEME_COLOR_DARK)};var lightMeta=${JSON.stringify(THEME_COLOR_LIGHT)};var stored=null;try{stored=localStorage.getItem(k);}catch(e){}var preference=stored==="light"||stored==="system"||stored==="dark"?stored:${JSON.stringify(DEFAULT_COLOR_SCHEME_PREFERENCE)};var systemLight=window.matchMedia("(prefers-color-scheme: light)").matches;var resolved=preference==="system"?(systemLight?"light":"dark"):preference;var root=document.documentElement;root.classList.toggle("light",resolved==="light");root.classList.toggle("dark",resolved!=="light");root.style.colorScheme=resolved;var meta=document.querySelector('meta[name="theme-color"]');if(meta)meta.setAttribute("content",resolved==="light"?lightMeta:darkMeta);}catch(e){}})();`;
}
