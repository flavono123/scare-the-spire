...
Do not commit `pnpm static:data` fallout (`public/generated/*` CI outputs, `src/generated/*.json`, `public/api/search-index`). Those are gitignored and regenerated by `pnpm static:data` / ensure helpers; after clone, run `pnpm static:data` once or rely on `dev` / `lint` / `patch:build` ensures.
