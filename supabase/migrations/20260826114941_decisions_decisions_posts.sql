-- 어려운 결정 posts plus toybox feed branch.
-- DB-first additive: table is unused until the local/new app writes.
-- `get_toybox_feed` gains a new p_service case; old callers keep the same
-- row shape. Do not expand `get_defragment_feed` here — production 조각모음
-- still indexes an exhaustive service map and would crash on the new
-- discriminator.
