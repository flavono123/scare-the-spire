...
as $$
  with own_activity as (
    ...
    select
      ...
      combo_posts.id::text as activity_id,
      0::bigint as like_count
    from public.combo_posts
    where combo_posts.env = p_env
      and combo_posts.user_id = (select auth.uid())
    ...
    select
      'history_course'::text as category,
      runs.id::text as activity_id,
      0::bigint as like_count
    from public.runs
    where runs.env = p_env
      and runs.donor_user_id = (select auth.uid())
  )
  select
...
as $$
  with own_activity as (
    ...
    select
      combo_posts.id::text as activity_id,
      'combo'::text as category,
      combo_posts.content_text as content,
      combo_posts.content as content_blocks,
      combo_posts.id::text as target_key,
      combo_posts.created_at,
      0::bigint as like_count
    from public.combo_posts
    where combo_posts.env = p_env
      and combo_posts.user_id = (select auth.uid())
    ...
    select
      runs.id::text as activity_id,
      'history_course'::text as category,
      runs.seed as content,
      null::jsonb as content_blocks,
      runs.id::text as target_key,
      runs.created_at,
      0::bigint as like_count
    from public.runs
    where runs.env = p_env
