-- Admin-only RLS activity metrics. DB-first additive: unused until /dev/admin
-- calls it with the service role. Public/anon/authenticated cannot execute.
--
-- This is not page-view analytics. It backfills identified-user visit/retention
-- from existing RLS user_id timestamps (posts, comments, likes, votes, plays,
-- inquiries) plus auth.users first/last session for those identities.
-- Bounce rate cannot be reconstructed: no page-view or session stream exists.
