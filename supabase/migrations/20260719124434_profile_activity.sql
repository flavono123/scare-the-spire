create index if not exists idx_comments_env_user_created
  on public.comments(env, user_id, created_at desc);

create index if not exists idx_community_stories_env_user_created
  on public.community_stories(env, user_id, created_at desc)
  where user_id is not null;

create index if not exists idx_chemical_posts_env_user_created
  on public.chemical_posts(env, user_id, created_at desc);

create index if not exists idx_this_or_that_posts_env_user_created
  on public.this_or_that_posts(env, user_id, created_at desc);

create or replace function public.get_my_profile_activity_stats(p_env text)
returns table(category text, post_count bigint, like_count bigint)
language sql
stable
security invoker
set search_path = ''
as $$
  ...
  select
    own_activity.category,
    count(*)::bigint as post_count,
    sum(own_activity.like_count)::bigint as like_count
  from own_activity
  group by own_activity.category;
$$;
...
revoke execute on function public.get_my_profile_activity_stats(text) from public, anon;
revoke execute on function public.get_my_profile_activity(text, text, text, integer, integer) from public, anon;
grant execute on function public.get_my_profile_activity_stats(text) to authenticated;
