...
- Enemy balance pass, including a rework of **Infested Prism**, more **Skulking Colony** adjustments, and **Aeonglass** changes.
- Added more Epoch art and new combat animations.

## Content & Balance

### General

- If an enemy has Vulnerable and a player takes an extra turn via **Pael's Eye**, the enemy's Vulnerable amount no longer decrements before the player's extra turn.
  - This change also applies to other powers that decrement at the end of the enemy's turn, such as Frail, Intangible, Weak, etc.

### Ancients

- Changed **Neow's Scroll Boxes** and **Silken Tress** relics: swapped downsides and pools.

### Ironclad

- **Drum of Battle** (buff): Energy gain on Exhaust effect now respects Replay and card play duplication effects like **Duplicator Potion**.
- **Howl From Beyond** (buff): trigger moved from start of turn -> end of turn.
- **Entrench** (buff): can now gain extra Block from enchantments like **Nimble**.
- **Unrelenting** (buff): damage increased from 12(18) -> 14(20).

### Silent

- Reworked and renamed **Follow Through**:
  - Rarity increased from Common -> Uncommon.
  - Renamed -> **Scare**.
- **Predator** (change): rarity decreased from Uncommon -> Common.
- **Pounce** (buff): damage increased from 12(18) -> 14(20).

### Regent

- **Furnace** (buff): Forge increased from 4(6) -> 5(7).
- **Minion Sacrifice** (nerf): Block decreased from 9(12) -> 8(11).
- **Sealed Throne** (change): upgrade changed from Innate -> lowering Energy cost by 1.
- **Astral Pulse** (change): damage changed from 14(18) -> 6x2(8x2).

### Necrobinder

- **Death March** (buff): additional damage increased from 3(4) -> 4(6).
- **Debilitate** (nerf): debuff decreased from 3(4) turns -> 2(3) turns.
...
- **Synthesis** (buff): damage increased from 12(18) -> 14(20).
- **Shatter** (nerf): damage decreased from 11(15) -> 7(11).

### Colorless Cards

- **Fasten** (nerf): additional Block decreased from 5(7) -> 4(6).

### Potions & Relics

- **Mummified Hand** (buff): now randomly chooses cards in a way that's more favorable to the player.
...
- Added the following enemies to the Bestiary:
  ...
  - **Decimillipede**
  - **Parafright**
- Added global rank paginator and adjusted arrow positions to leaderboards.
...
- Added controller navigation for the modifier icons on the top bar.
...
- Fixed num keys for card selection not working if your mouse is too low on the screen.
...
- Fixed broken controller navigation from relics to card rewards after you reroll via **Driftwood** relic.
- Fixed completing **Orrery** relic's card selection in shop also buying or clicking on an item in the shop on controller.
