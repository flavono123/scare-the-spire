#!/usr/bin/env python3
"""Parse PowerModel.ExtraHoverTips from decompiled STS2 sources into JSON.

Source of truth: /tmp/sts2-src MegaCrit.Sts2.Core.Models.Powers/*.cs
...
DEFAULT_SOURCE = Path("/tmp/sts2-src/MegaCrit.Sts2.Core.Models.Powers")
...
DEFAULT_OUTPUT = Path("data/sts2/power-extra-hover-tips.json")
...
def main() -> None:
    ...
    for path in sorted(args.source.glob("*.cs")):
        ...
        if "_extraHoverTips" in body or body.startswith("list"):
            ...
            continue
        power_id = _relic.power_id(path.stem)
        ...
        if specs:
            result[power_id] = specs

    payload = {
        "source": "MegaCrit.Sts2.Core.Models.Powers ExtraHoverTips",
        "tipsByPowerId": dict(sorted(result.items())),
        "skippedDynamicPowerClasses": skipped_dynamic,
    }

    if args.dry_run:
        print(json.dumps(payload, ensure_ascii=False, indent=2)[:2500])
        print(f"... {len(result)} powers, {len(skipped_dynamic)} dynamic skipped")
        return

    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n")
    print(f"wrote {args.output} ({len(result)} powers)")
