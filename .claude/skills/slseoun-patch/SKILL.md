...
Filter by `tags` containing `"patchnotes"`. **Note**: Major Updates and some stable releases may NOT have the `patchnotes` tag — always also search without tag filter if the target patch isn't found. Each item has:
- `gid` — unique ID
...
- Ascensions: 숫자 레벨만 태그한다. `[gold:ascension]승천 8[/gold]`, `[gold:ascension]Ascension 8[/gold]`, `[gold:ascension]A8[/gold]`. 맨몸 `승천` / `Ascension`은 태그하지 않는다 (튜토리얼, "높은 승천").

**판단 기준**: 패치노트 문맥에서 어떤 타입인지 확인하고, 해당 이름이 카드+파워 양쪽에 존재하면 타입 힌트를 붙인다. 패치노트는 카드 변경이 주류이므로 대부분 `[gold:card]`가 된다.

#### Ascension history (2단계)

승천의 패치 이력은 해당 승천에 **영향을 받는 변경을 모두** 포함한다. 모디파이어 자체 리워크만이 아니다.

- `승천 N` / `Ascension N` / `A8` 직접 언급은 그 레벨의 이력이다. 예: `갑각충 (버프): 승천 8 체력이 24-28(25-29) → 24-28(26-30)` 은 갑각충 이력이자 승천 8 이력이다.
- 승천 문구가 없는 경우도 있다. 대표적으로 몬스터 피해량의 괄호 값(고승천/A9)만 바뀌는 줄. 카드 강화 괄호 `6(8)` 와 혼동하지 말 것.
  - 몬스터 **체력** 괄호는 승천 8 (질긴 적들)
  - 몬스터 **피해량** 괄호는 승천 9 (지독한 적들)
- Steam 원문에 승천 문장이 없으면 한국어에 `승천 9`를 지어내지 않는다. 매칭은 `scripts/generate-sts2-patch-lines.ts`가 하고, 렌더러는 태그되지 않은 `승천 n`도 호버/링크로 감싼다.
...
#### Developer Notes (devnote)
Steam 원문에서 개발자의 디자인 의도, 변경 이유, 실험적 시도에 대한 코멘트를 추출하여 한국어 .ko.md 파일에만 포함한다. 영문 .md 파일에는 넣지 않는다.

**판별 기준**: 기계적 변경 설명("Damage 5 -> 7")이 아닌, 왜 그런 변경을 했는지("This change is aimed at improving survivability"), 실험적 시도인지("This is still an experiment"), 향후 계획("we will be looking to expand") 등 개발자의 주관적 코멘트.

**형식**:
```
[devnote]한국어 번역[/devnote]
[devnote:en]English original text from Steam[/devnote]
```

**배치 규칙**:
- 섹션 전체에 대한 코멘트: `###` 헤더 바로 아래
- 특정 항목에 대한 코멘트: 해당 항목 바로 아래
- 한국어 번역 → 영문 원문 순서

**번역 규칙**:
- 게임 엔티티 이름은 `[gold]`/`[gold:card]` BBCode 적용
- 자연스러운 한국어로 의역 (직역 금지)
- 원문의 뉘앙스(실험적, 확신, 기대 등)를 보존

**예시** (v0.103.0 아이언클래드 섹션):
```
### [red]아이언클래드[/red]
[devnote]아이언클래드의 생존력을 크게 개선하는 데 집중한 변경. [gold:card]거상[/gold]을 고급으로 내리면서 희귀 방어 옵션 의존도를 줄이고...[/devnote]
[devnote:en]This set of changes is directly aimed at improving Ironclad's survivability in a big way. Colossus moving to the Uncommon pool helps to lessen the need for Rare defensive options...[/devnote]
```
...
#### 7f. `{Var}` 베이크 갱신 — `/baking` 호출

수치/리워크가 들어가면 유물·포션·파워의 디폴트 `{Var}` 값이 바뀐다. PCK
디컴파일이 갱신된 상태에서 `/baking` 스킬을 실행해
`data/sts2/{eng,kor}/{relics,potions,powers}.json`의 `vars` 컬럼을 새로 채운다.
`vars`는 `src/lib/codex-bake.ts`가 런타임에 description으로 베이크하므로,
누락 시 사이트에 `{FocusPower}` 같은 템플릿이 그대로 노출된다.
