...
- `<details>` / `<summary>` is allowed for long expandable lists. Put `<details>` then `<summary>표시 문구</summary>`, then `-` bullets, then `</details>`. Version rows should be markdown links to `/patches/vX.Y.Z`. Game resource names and keywords in those bullets get game hover tips even without `[gold:…]` tags; typed gold tags still win when present.
