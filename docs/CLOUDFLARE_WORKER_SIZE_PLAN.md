...
- patch Worker binding, production `/patches/changes` 404, pending Compendium
...
- 변경 이력 explorer와 이를 포함한 patch 탭은 development-only다. 기본
  `pnpm dev`가 `STS_PATCH_CHANGES_DEV=1`로 정적 explorer를 생성하지만,
  production patch build는 `/patches/changes` HTML과 전용 client bundle을
  생성하지 않는다. 리소스 상세의 patch 이력 rail이 읽는
  `/generated/sts2-resource-patch-index.json`은 계속 build-time에 생성한다.
