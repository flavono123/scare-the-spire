...
Phase 4의 권위 있는 production-shaped local 검증은 `pnpm cf:phase4`다. 이
명령은 main/patch artifact를 빌드하고 두 local Worker와 service binding을
기동한 뒤 route smoke와 Playwright browser QA를 실행한다. 다음 순서로 최종
검증한다.
...
마지막 `pnpm cf:size`는 Phase 4를 통과한 정확한 main artifact를 다시
...
warning을 기록한다. Phase 4.1을 실행하거나 push/deploy하지 않고 여기서 멈춘다.
...
- Phase 2에서 공유 모듈로 옮긴 helper를 `page.tsx`의 비표준 export로 되돌리지
  않는다.
- Combo는 `/combo` locale URL과 metadata를 유지하고, `/combo/[id]` record를
  브라우저에서 Supabase로 읽는다. Worker 요청 시 Supabase 조회, 전체 리소스
  join, 대형 JSON 파싱을 추가하지 않는다.
- 변경 이력의 production owner는 별도 정적 patch Worker다. `/patches*`를 main
  OpenNext runtime으로 옮기지 않고, `/patches/changes` HTML/CSS/JS와
  `/generated/sts2-resource-patch-index.json`을 build-time에 생성한다.
